          ...
          <transition-group name="list" tag="g" class="depth">
            ...
            >
              ...
              <text
                ...
                :key="
                  't_' + children.id + (children.data[0] || children.data.type)
                "
                :x="x(children.x0) + 10"
                :y="y(children.y0) + 6"
                :style="{
                ...
                :style="{
                  'fill-opacity':
                    windowWidth >= breakWidth
                      ? y(children.y1 - children.y0 + children.parent.y0) >=
                          70 &&
                        x(children.x1 - children.x0 + children.parent.x0) >= 82
                        ? '1'
                        : '0'
                      : y(children.y1 - children.y0 + children.parent.y0) >=
                          62 &&
                        x(children.x1 - children.x0 + children.parent.x0) >= 50
                      ? '1'
                      : '0',
                ...
                >
                  <div class="label">
                    ...
                    <div style="padding-left: 1.5px">
                      <StackedBar year="2561" v-if="rawData" :data="rawData" />
                    </div>
                  </div>
                </Checkbox>
              </li>
...
export default {
  ...
  watch: {
    selectedNode(newData, oldData) {
      // console.log("The selected node changed...", this.selectedNode);
    },
    selectedYearCompute() {
      // console.log("year changed", this.selectedYear);
      if (this.selectedYear.length === 0) {
        this.selectedYear = ["2561", "2562", "2563", "2564", "2565"];
      }
      this.updateData();
      // this.initialize();
    },
  ...
  async mounted() {
    ...
    const budget = raw
      ...
      .map(data => ({
        ...
        expected: Math.round(data["งบประมาณตามแผน(ล้านบาท)"].replace(",", ""))
      }));
  ...
  methods: {
    updateData() {
      ...
      if (this.rawData) {
        ...
        // console.log(that.rootNode.data);
        // console.log({}.data);
      }
    },
    initialize() {
      ...
      if (that.nestedData) {
        // console.log("data", that.nestedData);
        that.rootNode = d3
        ...
        // console.log("sum", typeof that.rootNode);
      }
    },
    ...
    getWindowWidth(event) {
      ...
      if (currentWidth >= this.maxWidth) {
        ...
        this.windowWidth = currentWidth - 40;
      }
    },
